...
function encontrarPalabraMasCorta(listaDePalabras) {
    ...
    for (let i = 0; i < listaDePalabras.length; i++) {
        const palabraActual = listaDePalabras[i];
        if(palabraCorta.length > palabraActual.length)
        {
...
function enumerarLista(listaDePalabras) {
    ...
    const sinUltima = listaDePalabras.slice(0, -1);
    const ultima = listaDePalabras[listaDePalabras.length -1];
    const oracion = sinUltima.join(", ") + " y " + ultima + ".";
